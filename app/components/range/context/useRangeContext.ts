...
export const useRangeContext = (props: RangeProps): RangeContextValue => {
  ...
  const { min, max, step = 0.1, rangeValues } = props;
  ...
  const updateInputsText = useCallback(
    (handle: Exclude<ActiveHandle, null>) => {
      ...
      if (newInputsText[handle] === inputsText[handle]) {
        return;
      }
  ...
  return useMemo<RangeContextValue>(
    () => ({
      trackRef,
      value,
      setValue,
